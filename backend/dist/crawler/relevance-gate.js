...
const GATE_LABELS = [...TOOL_CATEGORIES, "not-ai"];
...
function buildUserPrompt(batch) {
    return JSON.stringify(batch.map((v) => ({
...
function parseLabels(raw, expected) {
    try {
        const cleaned = raw.trim().replace(/^```json/i, "").replace(/^```/, "").replace(/```$/, "").trim();
        const arr = JSON.parse(cleaned);
        if (!Array.isArray(arr) || arr.length !== expected)
            return null;
        return arr.map((label) => (GATE_LABELS.includes(label) ? label : "not-ai"));
    }
    catch {
        return null;
    }
...
async function classifyWithGroq(batch) {
 ...
 */
export async function gateVideos(candidates) {
