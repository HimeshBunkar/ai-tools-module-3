 ...
 */
export async function enrichVideos(videoIds) {
    const results = [];
    const batches = [];
    ...
    for (const batch of batches) {
        const url = `${API_BASE}/videos?part=snippet,contentDetails,statistics&id=${batch.join(",")}&key=${apiKey()}`;
        ...
        for (const item of json.items ?? []) {
            ...
            if (!isLikelyAiRelated(title, description))
                continue;
