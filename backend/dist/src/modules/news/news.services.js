...
export class NewsService {
     ...
     */
    withTrendingScores(rows, bookmarkedIds = new Set()) {
    ...
    async loadBookmarkedIds(articleIds, clientId) {
        if (!clientId || articleIds.length === 0)
            return new Set();
     ...
     * scores.
     */
    async getArticles(paging, clientId) {
        ...
        const [rows, total] = await Promise.all([
            withTiming("getArticles db query (paginated)", () => this.prisma.news.findMany({
                include: ARTICLE_INCLUDE,
                orderBy,
                skip: (page - 1) * perPage,
                take: perPage,
    ...
    async getArticleBySlug(slug, clientId) {
