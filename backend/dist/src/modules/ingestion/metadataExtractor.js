...
function extractBodyExcerpt(html) {
    ...
    $("script, style, nav, header, footer, aside, noscript, svg, form, iframe, button, " +
        ...
        ".video-player, .ad, .advertisement, .comments").remove();
...
export function extractArticleMetadata(html) {
    ...
    const authorField = jsonLd?.author;
    ...
    const publisherField = jsonLd?.publisher;
    const publisherLogo = typeof publisherField?.logo === "string" ? publisherField.logo : publisherField?.logo?.url ?? null;
    return {
        headline: cleanText(jsonLd?.headline || og["og:title"]) || null,
        description: cleanText(jsonLd?.description || og["og:description"] || og["description"]) || null,
        datePublished: jsonLd?.datePublished || og["article:published_time"] || null,
        ...
        imageUrl: jsonLd?.image || og["og:image"] || null,
