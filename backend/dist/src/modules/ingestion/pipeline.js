...
import { generateAiSummary } from "./llmSummarizer.js";
...
export async function loadRecentTitleIndex(prisma) {
 ...
 */
async function validateEntry(prisma, entry, source, titleIndex, seenInBatch) {
    const articleUrl = normalizeUrl(entry.link);
    if (!articleUrl)
        return { kind: "invalid" };
    if (seenInBatch.has(articleUrl))
    ...
    if (isNearDuplicateTitle(titleIndex, title))
        return { kind: "near-duplicate" };
 ...
 */
async function enrichValidated(v) {
    ...
    if (!publishedAt || isThinSummary(summary)) {
        ...
        if (!summary && enriched.description)
...
function withinSummarizationBudget() {
    return Date.now() - PIPELINE_STARTED_AT < SUMMARIZATION_BUDGET_MS;
}
async function summarizePrepared(p, llmKeys) {
...
async function finalizePrepared(prisma, cloudinary, p, aiSummary, source) {
    const publisher = await resolvePublisher(prisma, cloudinary, p.domain, p.nameHint);
...
function newPipelineResult(source) {
    return {
        source,
        fetched: 0,
        created: 0,
...
async function ingestEntries(ctx, entries, source, titleIndex, runCap) {
    ...
    for (const entry of entries) {
        try {
            ...
            else if (outcome.kind === "near-duplicate")
                result.skippedNearDuplicate++;
            else if (outcome.kind === "not-relevant")
                result.skippedNotAiRelevant++;
            else
    ...
    const prepared = await mapWithConcurrency(validated, ENRICH_CONCURRENCY, async (v) => {
        try {
            ...
            result.errors.push(`"${v.title}" (enrichment): ${err.message}`);
    ...
    const summaries = await mapWithConcurrency(prepared, SUMMARY_CONCURRENCY, async (p) => {
        try {
            ...
            result.errors.push(`"${p.title}" (summarization): ${err.message}`);
