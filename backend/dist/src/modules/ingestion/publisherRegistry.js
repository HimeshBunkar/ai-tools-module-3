...
function guessDisplayName(domain) {
 ...
 */
async function resolveLogo(cloudinary, domain, sourceKey) {
    const faviconUrl = `https://${domain}/favicon.ico`;
    const bundled = sourceKey ? SOURCE_LOGOS[sourceKey] : undefined;
    if (bundled)
        return { logoUrl: `/logos/${bundled}`, faviconUrl };
    if (cloudinary) {
        const downloaded = await resolvePublisherLogo(cloudinary, domain);
        if (downloaded)
            return { logoUrl: downloaded, faviconUrl };
    }
 ...
 * already know about.
 */
export async function resolvePublisher(prisma, cloudinary, domain, nameHint) {
...
function cleanNameHint(nameHint) {
    if (!nameHint)
        return null;
