...
async function tryGemini(prompt, apiKey, debug) {
    ...
    try {
        ...
        debugLog(debug, `Gemini -> exception: ${err.name}: ${err.message}`);
        return FAILED;
    }
}
/** Tier 2: Groq free tier — OpenAI-compatible chat completions API. */
async function tryGroq(prompt, apiKey, debug) {
    try {
...
async function tryPollinations(prompt, debug) {
    try {
        ...
        debugLog(debug, `Pollinations -> exception: ${err.name}: ${err.message}`);
        return FAILED;
    }
}
/** Waterfall: Gemini -> Groq -> Pollinations, retried a couple of times with a short backoff between full passes. */
async function generateJsonGated(prompt, keys) {
    ...
    for (let attempt = 0; attempt < WATERFALL_RETRIES; attempt++) {
        ...
        if (result)
            return result;
        if (attempt < WATERFALL_RETRIES - 1)
 ...
 */
export async function generateAiSummary(title, description, keys = {}) {
