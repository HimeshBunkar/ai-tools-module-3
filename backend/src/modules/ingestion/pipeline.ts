...
import type { CloudinaryConfig } from "./logoResolver.js";

export interface IngestionContext {
  prisma: PrismaClient;
  llmKeys: SummarizerKeys;
  /**
   * Optional — undefined if Cloudinary credentials aren't configured.
   * publisherRegistry.ts's resolveLogo() degrades gracefully to the
   * bundled-SVG-or-favicon tiers when this is absent.
   */
...
async function finalizePrepared(prisma: PrismaClient, cloudinary: CloudinaryConfig | undefined, p: PreparedEntry, aiSummary: string, source: FeedSource): Promise<void> {
