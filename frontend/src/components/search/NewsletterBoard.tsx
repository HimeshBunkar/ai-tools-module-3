...
function PostCard({ post, tall = false }: { post: Post; tall?: boolean }) {
  return (
    <article className="overflow-hidden rounded-lg border border-search-border bg-search-surface transition-colors hover:border-search-border-hover">
      <PostThumbnail post={post} tall={tall} />
      <div className="p-3.5">
        <div className="mb-1.5 flex items-center gap-1.5 text-[11px] text-search-text-tertiary">
          <Clock size={10} />
          {post.readMinutes} min read · {post.date}
        </div>
        <h3 className="mb-1.5 line-clamp-2 text-sm font-semibold text-search-accent-hover hover:text-search-accent">
          {post.headline}
        </h3>
        <p className="mb-3 line-clamp-2 text-xs text-search-text-secondary">{post.excerpt}</p>
        <div className="flex items-center gap-1.5 border-t border-search-border pt-2.5 text-[11px] text-search-text-tertiary">
          <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-search-accent-soft text-search-accent">
            <Sparkles size={9} />
          </span>
          The AI Signal
        </div>
...
export function NewsletterBoard() {
  ...
  return (
    <div>
      {/* Hero / subscribe */}
      <section className="hero-glow mb-10 -mx-4 rounded-xl border border-search-border bg-search-surface px-4 py-10 text-center sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
        <span className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-search-accent-soft text-search-accent">
          <Mail size={18} />
        </span>
        <h1 className="brand-wordmark text-3xl text-search-text-primary sm:text-4xl">The&nbsp;AI&nbsp;Signal</h1>
        <p className="mx-auto mt-3 max-w-xl text-sm text-search-text-secondary">
        ...
        >
          <input
            ...
            className="w-full rounded-md border border-search-border bg-search-bg px-3.5 py-2.5 text-sm text-search-text-primary outline-none placeholder:text-search-text-tertiary hover:border-search-border-hover focus:border-search-border-hover"
          />
          <button
            type="submit"
            className="shrink-0 whitespace-nowrap rounded-md bg-search-highlight px-4 py-2.5 text-sm font-semibold text-search-highlight-text transition-colors hover:brightness-110"
          >
            Join 2.5M+ readers
          </button>
        </form>
        <p className="mx-auto mt-3 max-w-md text-[11px] text-search-text-tertiary">
      ...
      {/* Featured posts */}
      <div className="mb-4 text-xs font-semibold uppercase tracking-wide text-search-text-tertiary">
        Featured Posts
      </div>
      ...
      <div className="relative mb-5 max-w-sm">
        <SearchIcon
          size={14}
          className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-search-text-tertiary"
        />
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Search"
          className="w-full rounded-md border border-search-border bg-search-surface py-2 pl-8 pr-3 text-sm text-search-text-primary outline-none placeholder:text-search-text-tertiary hover:border-search-border-hover focus:border-search-border-hover"
        />
      </div>
