...
function LocationIcon({ type }: { type: LocationType }) {
  if (type === "On-site") return <Building2 size={11} className="text-search-text-tertiary" />;
  if (type === "Hybrid") return <Home size={11} className="text-search-text-tertiary" />;
  return <Globe2 size={11} className="text-search-text-tertiary" />;
}
  ...
  return (
    <div>
      ...
      <div className="mb-5 flex items-center justify-between">
        <h1 className="text-xl font-semibold text-search-text-primary">Job Listings</h1>
        <button
          type="button"
          className="inline-flex items-center gap-1.5 rounded-md bg-search-accent px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-search-accent-hover"
        >
          <Plus size={14} />
          Create Job
        </button>
      ...
      {/* Filters panel */}
      <div className="mb-6 rounded-xl border border-search-border bg-search-surface p-4">
        <button
          type="button"
          onClick={() => setFiltersOpen((v) => !v)}
          className="flex w-full items-center gap-2 text-sm font-medium text-search-text-primary"
        >
        ...
        {filtersOpen && (
          <div className="mt-4 space-y-4">
            <div className="grid gap-4 sm:grid-cols-3">
              <div>
                <label className="mb-1.5 block text-xs text-search-text-tertiary">Position</label>
                <input
                  value={positionInput}
                  onChange={(e) => setPositionInput(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && runSearch()}
                  placeholder="Position"
                  className="w-full rounded-md border border-search-border bg-search-bg px-3 py-2 text-sm text-search-text-primary outline-none placeholder:text-search-text-tertiary hover:border-search-border-hover focus:border-search-border-hover"
                />
              </div>
              <div>
                <label className="mb-1.5 block text-xs text-search-text-tertiary">Company</label>
                <input
                  value={companyInput}
              ...
              <div>
                <label className="mb-1.5 block text-xs text-search-text-tertiary">Location Type</label>
                    ...
                    return (
                      <button
                        ...
                        className={`inline-flex items-center gap-1.5 rounded-md border px-3 py-2 text-xs font-medium transition-colors ${
                          active
                            ? "border-search-accent bg-search-accent-soft text-search-accent"
                            : "border-search-border bg-search-bg text-search-text-secondary hover:border-search-border-hover hover:text-search-text-primary"
                        }`}
                      >
                        <Icon size={13} />
                        {opt.value}
            ...
            <button
              type="button"
              onClick={runSearch}
              className="flex w-full items-center justify-center gap-2 rounded-md bg-search-text-primary py-2.5 text-sm font-semibold text-search-bg transition-colors hover:bg-search-accent hover:text-white"
            >
              <SearchIcon size={14} />
              Search Jobs
            </button>
      ...
      {/* Results */}
      <p className="mb-3 text-xs text-search-text-tertiary">{filtered.length} jobs</p>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {filtered.map((job) => (
            <div
              key={job.id}
              className="flex flex-col rounded-lg border border-search-border bg-search-surface p-4 transition-colors hover:border-search-border-hover"
            >
              <div className="mb-3 flex items-start gap-2.5">
                <JobLogo text={job.logoText} color={job.logoColor} />
                <div className="min-w-0">
                  <h3 className="truncate text-sm font-semibold text-search-text-primary">{job.title}</h3>
                  <p className="truncate text-xs text-search-text-secondary">{job.company}</p>
                </div>
              </div>

              <div className="flex flex-col gap-1.5 text-xs text-search-text-secondary">
                ...
                <span className="inline-flex items-center gap-1.5">
                  <DollarSign size={11} className="text-search-text-tertiary" />
                  {formatSalary(job.salaryMin)} - {formatSalary(job.salaryMax)}
                </span>
              </div>

              <div className="mt-3 flex items-center justify-between border-t border-search-border pt-2.5">
                <span className="rounded-md bg-search-surface-active px-2 py-1 text-[11px] font-medium text-search-text-primary/80">
                  {job.employmentType}
                </span>
                <span className="text-[11px] text-search-text-tertiary">
