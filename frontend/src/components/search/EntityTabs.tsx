...
export function EntityTabs({ selected, onChange, counts }: EntityTabsProps) {
  ...
  return (
    <div className="relative">
      {canScrollLeft && (
        <>
          <div className="pointer-events-none absolute left-0 top-0 z-10 h-full w-10 bg-gradient-to-r from-[var(--color-search-bg)] to-transparent" />
          <button
            aria-label="Scroll categories left"
            onClick={() => scrollBy(-160)}
            className="absolute left-0.5 top-1/2 z-20 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-full border border-search-border bg-search-surface text-search-text-secondary shadow-md hover:text-search-text-primary"
          >
            <ChevronLeft size={14} />
          </button>
        </>
      ...
      {canScrollRight && (
        <>
          <div className="pointer-events-none absolute right-0 top-0 z-10 h-full w-10 bg-gradient-to-l from-[var(--color-search-bg)] to-transparent" />
          <button
            aria-label="Scroll categories right"
            onClick={() => scrollBy(160)}
            className="absolute right-0.5 top-1/2 z-20 flex h-6 w-6 -translate-y-1/2 items-center justify-center rounded-full border border-search-border bg-search-surface text-search-text-secondary shadow-md hover:text-search-text-primary"
          >
