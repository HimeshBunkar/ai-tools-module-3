...
function CheckboxGroup({ title, options, selected, onChange, counts, collapseAfter }: CheckboxGroupProps) {
  ...
  return (
    <div className="mb-5">
      <div className="mb-3 border-b border-search-border pb-3">
        <h3 className="text-sm font-semibold tracking-tight text-search-text-primary">{title}</h3>
      </div>
      <div className="space-y-0.5 pt-1">
        {visible.map((option) => (
          <label
            key={option}
            className="flex cursor-pointer items-center justify-between gap-2.5 rounded-md px-1.5 py-1.5 text-sm text-search-text-secondary hover:bg-search-surface-hover"
          >
            <span className="flex items-center gap-2.5">
              <input
                ...
                className="h-3.5 w-3.5 rounded border-search-border-hover accent-[var(--color-search-accent)]"
              />
              {option}
            </span>
            {counts && (
      ...
      {collapseAfter && options.length > collapseAfter && (
        <button
          ...
          className="mt-1.5 px-1.5 text-xs font-medium text-search-accent hover:underline"
        >
...
function PriceRangeFilter({ bounds, value, onChange }: PriceRangeProps) {
  ...
  return (
    <div className="mb-5">
      <div className="mb-3 flex items-center justify-between border-b border-search-border pb-3">
        <h3 className="text-sm font-semibold tracking-tight text-search-text-primary">Price / month</h3>
        <span className="text-xs tabular-nums text-search-text-tertiary">
          ${min} – ${max === bounds.max ? `${max}+` : max}
        </span>
      </div>
      <div className="px-1.5 pt-2">
        <div className="relative h-1.5 rounded-full bg-search-surface-active">
          <div
            className="absolute h-1.5 rounded-full bg-search-accent"
            style={{
        ...
        <div className="mt-3 flex items-center justify-between text-xs text-search-text-tertiary">
          <span>Free</span>
  ...
  return (
    <div className="pr-2">
      <div className="mb-3 flex items-center justify-between">
        <span className="text-xs font-semibold uppercase tracking-wide text-search-text-tertiary">Filters</span>
        {hasAnyActive && (
          <button onClick={resetAll} className="text-xs text-search-text-tertiary hover:text-search-text-primary">
            Reset all
          </button>
        )}
      </div>
