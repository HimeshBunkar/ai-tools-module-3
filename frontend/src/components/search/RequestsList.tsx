    ...
    <div>
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-search-text-primary">AI Tool Requests</h1>
            <p className="max-w-md text-sm text-search-text-secondary">
        ...
        <div className="flex shrink-0 items-center gap-4 rounded-xl border border-search-border bg-search-surface px-5 py-3">
          ...
          <button
            type="button"
            className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-md bg-search-text-primary px-3.5 py-2 text-xs font-semibold text-search-bg transition-colors hover:bg-search-accent hover:text-white"
          >
            <Plus size={13} />
            View request
          </button>
      ...
      <div className="mb-5 flex flex-wrap items-center justify-center gap-3">
          ...
          {TABS.map((t) => (
            <button
              ...
              className={`rounded-full px-3.5 py-1.5 text-sm transition-colors ${
                tab === t ? "bg-search-text-primary text-search-bg" : "text-search-text-secondary hover:text-search-text-primary"
              }`}
            >
              {t}
            </button>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-56 rounded-full border border-search-border bg-search-surface py-1.5 pl-8 pr-3 text-xs text-search-text-primary outline-none placeholder:text-search-text-tertiary hover:border-search-border-hover focus:border-search-border-hover"
          />
        </div>
      </div>
        ...
        {filtered.map((entity) => {
          ...
          return (
            <div key={entity.id} className="flex items-stretch gap-3">
              {/* Vote box */}
              <div className="flex w-24 shrink-0 flex-col items-center justify-center gap-1 rounded-xl border border-search-border bg-search-surface px-2 py-4 text-center">
                <p className="text-2xl font-bold leading-none tabular-nums text-search-text-primary">
                  {displayVotes}
                </p>
                <p className="text-[11px] text-search-text-tertiary">{displayVotes === 1 ? "vote" : "votes"}</p>
                <p className="text-[11px] text-search-text-tertiary">
                ...
                <button
                  ...
                  className={`mt-2 w-full rounded-md px-2 py-1.5 text-[11px] font-bold uppercase tracking-wide transition-colors ${
                    voted
                      ? "bg-search-warning/40 text-search-warning"
                      : "bg-search-warning text-black hover:bg-search-warning/90"
                  }`}
                >
                  Vote
                </button>
              ...
              {/* Content card */}
              <div className="group flex min-w-0 flex-1 items-center rounded-xl border border-search-border bg-search-surface p-4 transition-colors hover:border-search-border-hover">
                <div className="min-w-0 flex-1">
                  <Link href="#" className="block">
                    <h3 className="text-[17px] font-bold leading-snug text-search-text-primary group-hover:text-search-accent">
                      {entity.title}
                    </h3>
                  </Link>

                  <div className="mt-2.5 flex flex-wrap items-center gap-2 text-xs text-search-text-tertiary">
                    <span className="rounded-full bg-search-surface-active px-2.5 py-1 text-[11px] font-medium text-search-text-secondary">
                      AI Tools
                    </span>
                    <span className="inline-flex items-center gap-1.5">
                      ...
                      <span className="text-search-text-secondary">{authorName}</span>
                    </span>
                    <span>· {timeLabel}</span>
                  </div>
                </div>
