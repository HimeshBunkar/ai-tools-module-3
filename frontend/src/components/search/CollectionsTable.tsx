    ...
    <div className="flex items-center">
      <div className="flex -space-x-2">
        {hues.map((hue, i) => (
          <span
            key={i}
            className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md border-2 border-search-surface text-[10px] font-semibold text-white"
            style={{ backgroundColor: `hsl(${hue}, 55%, 45%)` }}
            aria-hidden
          >
            {letters[i]}
      ...
      {remaining > 0 && (
        <span className="ml-2 whitespace-nowrap text-xs text-search-text-tertiary">+{remaining} more</span>
      )}
    </div>
  );
}
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1 text-xs font-medium uppercase tracking-wide transition-colors ${
        active ? "text-search-text-primary" : "text-search-text-tertiary hover:text-search-text-secondary"
    ...
    <div>
      ...
      <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-search-text-primary">Collections</h1>
            <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-search-text-secondary">
              <span>
                Collections <span className="font-semibold text-search-text-primary">{formatCount(total)}</span>
              </span>
              <span>
                Total tools <span className="font-semibold text-search-text-primary">{formatCount(totalTools)}</span>
              </span>
              <span>
                Subscribers <span className="font-semibold text-search-text-primary">{formatCount(totalSubscribers)}</span>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 text-xs text-search-text-tertiary">
            <span>Show:</span>
          ...
          <button className="inline-flex items-center gap-1.5 rounded-md bg-search-accent px-3 py-1.5 text-xs font-medium text-white hover:bg-search-accent-hover">
            <Plus size={13} />
            New
          </button>

          <div className="inline-flex rounded-md border border-search-border bg-search-surface p-1">
            <button
              onClick={() => setView("list")}
              aria-label="List view"
              aria-pressed={view === "list"}
              className={`rounded px-1.5 py-1 transition-colors ${
                view === "list" ? "bg-search-surface-active text-search-text-primary" : "text-search-text-tertiary hover:text-search-text-secondary"
              }`}
            >
              <List size={14} />
            </button>
            <button
              ...
              className={`rounded px-1.5 py-1 transition-colors ${
                view === "grid" ? "bg-search-surface-active text-search-text-primary" : "text-search-text-tertiary hover:text-search-text-secondary"
              }`}
            >
              <LayoutGrid size={14} />
            </button>
      ...
      <div className="overflow-x-auto rounded-lg border border-search-border bg-search-bg shadow-sm">
        <table className="w-full min-w-[860px] table-fixed border-collapse text-sm">
          <thead>
            <tr className="border-b border-search-border bg-search-surface text-left">
              <th className="w-[4%] px-4 py-3 text-xs font-medium uppercase tracking-wide text-search-text-tertiary">
                #
              </th>
              ...
              <th className="w-[16%] px-3 py-3 text-xs font-medium uppercase tracking-wide text-search-text-tertiary">
                Author
              </th>
              <th className="w-[28%] px-3 py-3 text-xs font-medium uppercase tracking-wide text-search-text-tertiary">
                Tools
              </th>
          ...
          <tbody>
            {sorted.map(({ entity, author, toolCount, stackHues, stackLetters, subscribers }, index) => {
              ...
              return (
                <tr key={entity.id} className="group border-b border-search-border last:border-b-0 hover:bg-search-surface-hover">
                  <td className="px-4 py-3 align-middle tabular-nums text-search-text-tertiary">{index + 1}</td>
                  <td className="px-2 py-3 align-middle">
                    <Link href={href} className="flex items-center gap-2.5">
                      ...
                      <span className="truncate text-sm font-medium text-search-accent group-hover:text-search-accent-hover">
                        {entity.title}
                      </span>
                    </Link>
                  </td>
                  <td className="px-3 py-3 align-middle">
                    <span className="flex items-center gap-2 text-sm text-search-text-secondary">
                  ...
                  <td className="px-3 py-3 align-middle text-search-text-secondary">
                    <span className="inline-flex items-center gap-1.5">
                      <Bell size={12} className="text-search-text-tertiary" />
                      {subscribers}
                    </span>
                  </td>
                  <td className="px-3 py-3 align-middle text-right text-xs text-search-text-tertiary">
