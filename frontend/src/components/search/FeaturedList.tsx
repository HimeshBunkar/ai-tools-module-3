    ...
    <div className="flex flex-col gap-3">
      {items.map((entity) => {
        ...
        return (
          <div
            key={entity.id}
            className="group relative overflow-hidden rounded-lg border border-search-border bg-search-surface transition-colors hover:border-search-border-hover"
          >
            {discount > 0 && (
              <span className="absolute left-0 top-0 rounded-br-lg bg-search-highlight px-2.5 py-1 text-[11px] font-semibold text-search-highlight-text">
                {discount}% OFF
              </span>
            )}
            ...
            <div className="flex flex-col gap-3 p-4 pt-4 sm:flex-row sm:items-center">
              ...
              <div className="min-w-0 flex-1">
                <Link href={href} className="flex min-w-0 items-center gap-1.5">
                  ...
                  {verified && (
                    <BadgeCheck size={15} className="shrink-0 text-search-accent" aria-label="Verified" />
                  )}
                </Link>

                <p className="mt-0.5 line-clamp-1 text-sm text-search-text-secondary">{entity.description}</p>
                ...
                <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1.5">
                  ...
                  <span className="text-xs text-search-text-tertiary">{entity.country}</span>
                  <span className="text-xs text-search-text-tertiary">
                    Released {monthsAgo === 0 ? "<1mo" : `${monthsAgo}mo`} ago
                  </span>
                </div>
              </div>

              <div className="flex shrink-0 items-center justify-between gap-4 border-t border-search-border pt-3 sm:justify-end sm:gap-5 sm:border-t-0 sm:pt-0">
                <div className="flex items-center gap-3 text-xs text-search-text-tertiary">
                ...
                <div className="text-right">
                  <p className={isFree ? "text-sm font-medium text-emerald-400" : "text-sm font-medium text-search-text-secondary"}>
                    {isFree ? "Free +" : "Paid"}
                  </p>
                  <p className="text-xs text-search-text-tertiary">from ${priceFrom}/mo</p>
                </div>
              </div>
            </div>
          </div>
