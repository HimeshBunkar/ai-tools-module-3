...
export function TasksTable({ items, total }: TasksTableProps) {
  ...
  return (
    <div>
      ...
      {/* Show: pills */}
      <div className="mb-4 flex items-center gap-2.5">
        <span className="text-xs text-search-text-tertiary">Show:</span>
          ...
          {SHOW_TABS.map((t) => (
            <button
              ...
              className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
                showTab === t ? "bg-search-accent text-white" : "text-search-text-secondary hover:text-search-text-primary"
              }`}
            >
              {t}
            </button>
      ...
      {/* Table */}
      <div className="overflow-x-auto rounded-lg border border-search-border bg-search-surface">
        <table className="w-full min-w-[860px] border-collapse text-sm">
          <thead>
            <tr className="border-b border-search-border text-left text-[11px] font-medium uppercase tracking-wide text-search-text-tertiary">
              <th className="w-10 px-4 py-3">#</th>
              <th className="px-2 py-3">
                <span className="inline-flex items-center gap-1">
                  Task
                  <ChevronsUpDown size={11} className="text-search-text-tertiary/60" />
                </span>
              </th>
              {COLUMNS.map((col) => {
                const Icon = col.icon;
                  ...
                  <th key={col.key} className="px-3 py-3 text-right">
                    <button
                      ...
                      className={`inline-flex items-center gap-1 transition-colors ${
                        active ? "text-search-text-primary" : "hover:text-search-text-secondary"
                      }`}
                    >
                      <Icon size={11} className="text-search-text-tertiary/70" />
                      {col.label}
                      {active ? (
                        sort.dir === "desc" ? (
                          <ChevronDown size={11} />
          ...
          <tbody>
            {sorted.map((entity, index) => {
              ...
              return (
                ...
                >
                  <td className="px-4 py-2.5 align-middle tabular-nums text-search-text-tertiary">
                    {index + 1}
                  </td>
                  <td className="px-2 py-2.5 align-middle">
                    <Link href={href} className="flex items-center gap-2">
                      <TaskIcon entity={entity} />
                      <span className="truncate text-sm font-medium text-search-accent-hover group-hover:text-search-accent">
                        {entity.title}
                      </span>
                    </Link>
                  </td>
                  <td className="px-3 py-2.5 align-middle text-right tabular-nums text-search-text-secondary">
                    <span className="inline-flex items-center gap-1.5 justify-end">
                      <Users size={11} className="text-search-text-tertiary" />
                      {formatCompact(m.subscribers)}
                    </span>
                  </td>
                  <td className="px-3 py-2.5 align-middle text-right tabular-nums text-search-text-primary">
                    <span className="inline-flex items-center gap-1.5 justify-end">
                      <Bookmark size={11} className="text-search-text-tertiary" />
                      {formatCount(m.saves)}
                    </span>
                  </td>
                  <td className="px-3 py-2.5 align-middle text-right tabular-nums text-search-text-secondary">
