    ...
    <div className="flex flex-col gap-3">
      {items.map((entity) => {
        ...
        return (
          <div
            key={entity.id}
            className="relative rounded-lg border border-search-border bg-search-surface p-4 transition-colors hover:border-search-border-hover"
          >
            <button
              onClick={() => toggleBookmark(entity.id)}
              aria-label={isSaved ? "Remove bookmark" : "Add bookmark"}
              aria-pressed={isSaved}
              className="absolute right-4 top-4 text-search-text-tertiary hover:text-search-text-primary"
            >
              <Bookmark size={16} className={isSaved ? "fill-search-accent text-search-accent" : ""} />
            </button>
            ...
            <div className="mt-3 flex gap-2 overflow-x-auto scrollbar-none">
              {images.map((src, i) => (
                <div
                  key={i}
                  className="relative h-20 w-20 shrink-0 overflow-hidden rounded-md bg-search-surface-active"
                >
            ...
            {/* Reviewer row */}
            <div className="mt-3 flex items-center gap-2 text-xs text-search-text-secondary">
              <ReviewerAvatar name={reviewer} />
              <span className="font-medium text-search-text-primary">{reviewer}</span>
              <span className="inline-flex items-center gap-0.5 text-search-text-tertiary">
                <Zap size={11} />
                {karma}
              </span>
            </div>
            <p className="mt-1.5 line-clamp-2 text-sm text-search-text-secondary">{quote}</p>
            <div className="mt-1.5 flex items-center gap-3 text-xs text-search-text-tertiary">
            ...
            {/* Footer */}
            <div className="mt-3 flex flex-wrap items-center justify-between gap-x-4 gap-y-1.5 border-t border-search-border pt-2.5 text-xs">
              <div className="flex items-center gap-3">
                <span
                  className={
                    pricing === "Free"
                      ? "font-medium text-emerald-400"
                      : "font-medium text-search-text-secondary"
                  }
                >
                  {pricing}
                </span>
                <span className="text-search-text-tertiary">
                  Released {releasedYearsAgo === 0 ? "<1y" : `${releasedYearsAgo}y`} ago
                </span>
              </div>
              <div className="flex items-center gap-3 text-search-text-tertiary">
                <span className="inline-flex items-center gap-1">
                  <Eye size={12} />
                  {formatCompact(views)}
                </span>
