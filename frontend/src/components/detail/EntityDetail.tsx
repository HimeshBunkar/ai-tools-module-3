...
export async function EntityDetail({ type, slug }: { type: EntityType; slug: string }) {
  ...
  return (
    <main className="search-scope mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
      <Link
        href="/search"
        className="mb-6 inline-flex items-center gap-1.5 text-sm text-search-text-secondary hover:text-search-text-primary"
      >
        <ArrowLeft size={15} />
        Back to search
      </Link>
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-search-text-tertiary">
            {meta.label}
          </p>
          <h1 className="mt-0.5 text-2xl font-semibold text-search-text-primary">{entity.title}</h1>
        </div>
      </div>

      <p className="mt-6 text-sm leading-relaxed text-search-text-secondary">{entity.description}</p>
      ...
      <div className="mt-8 grid grid-cols-2 gap-4 rounded-lg border border-search-border bg-search-surface p-5 sm:grid-cols-3">
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="text-xs text-search-text-tertiary">{label}</p>
      <p className="mt-0.5 truncate text-sm text-search-text-primary">{value}</p>
    </div>
  );
}
